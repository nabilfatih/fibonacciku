...
Answer the question based only on the following context and chat history: (If there is no chat history, say Hi (warm greeting) to the user first.)
<context>
  {context}
</context>
...
export async function POST(req: NextRequest) {
  ...
  const { fileId, messages } = (await req.json()) as {
    fileId: string;
    messages: any[];
  };

  // get only the last 10 messages
  const finalMessage = messages.slice(-10);
